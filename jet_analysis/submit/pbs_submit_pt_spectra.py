...
execpath = os.getcwd()
  ...
  while checkstatus(jobstatus) :
    ...
    for i in range(len(jobstatus)) :
      ...
      clargs = clargs + ' --efficiencyCurves=' + args.efficiency + ' --nhitsfit=' + args.nhitsfit
      clargs = clargs + ' --eta=' + args.eta + ' --dca=' + args.dca + ' --fitfrac=' + args.fitfrac
      clargs = clargs + ' --year14=' + args.year14 + ' --year7=' + args.year7
...
if __name__ == "__main__":
  ...
  parser.add_argument('--nhitsfit', type=int, default=15, help='min number of fit points required for a track')
  parser.add_argument('--eta' type=float, default=1.0, help='defines eta range for tracks from -eta to +eta')
  parser.add_argument('--dca', type=float, default=2.0, help='defines maximum DCA')
  parser.add_argument('--fitfrac', type=float, default=0.5, help='defines minimum nhits/nhitspossible')
