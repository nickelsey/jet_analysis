...
int main(int argc, char* argv[]) {
  ...
  if (opts.effic_file.empty())
    efficiency = new Run7Eff();
  else
    efficiency = new Run7Eff(opts.effic_file);
  
  switch(opts.track_unc) {
    case 0 :
      efficiency->setSystematicUncertainty(TrackingUncY7::NONE);
      break;
    case 1 :
      efficiency->setSystematicUncertainty(TrackingUncY7::POSITIVE);
      break;
    case -1 :
      efficiency->setSystematicUncertainty(TrackingUncY7::NEGATIVE);
      break;
    default:
  ...
  try {
    while (reader->NextEvent()) {
      ...
      // if no efficiency, copy the whole thing
      else {
